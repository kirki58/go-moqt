...
- `pkg/message/` directory contains the wire encoding and decoding logic of the protocol's objects
- `pkg/session/` contains code that are related to the management of a MOQT session between 2 pairs (control plane) and the logic to stream and receive track data (data plane)
...
- `server.go` and `client.go` under `peer` package provides logic for both types of peers to perform MOQT communication at the very high-level, They both use abstractions, functions, structs etc. defined under `pkg/`
